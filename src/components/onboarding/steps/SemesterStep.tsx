...
export function SemesterStep() {
  const { user } = useAuth();
  const { state, updateSemesterData, setCreatedSemesterId, setError } = useOnboarding();
  ...
  const createSemester = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setIsCreating(false);
        // The navigation will be handled by the StepNavigation component
      }, 500);
  ...
  // Get current date for input defaults
    ...
    <div className="max-w-lg mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-light-text-secondary dark:text-dark-text-secondary">
          Set up a semester to organize your courses and assessments. You can add more semesters later.
        </p>
      </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-light-text-secondary dark:text-dark-text-secondary">
                Don&apos;t worry about getting everything perfect right now. You can always edit semester details 
                or create additional semesters from your dashboard later.
              </p>
            </div>
          </div>
        </div>
