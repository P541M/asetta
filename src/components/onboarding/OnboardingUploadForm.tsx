...
export function OnboardingUploadForm({
  ...
  const handleUpload = async () => {
    if (!user || !fileInputRef.current?.files?.length) return;
          ...
          <div>
            ...
            <div className="space-y-2 mb-4">
              {files.map((file, index) => (
                <div key={index} className="flex items-center justify-between bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-lg p-3">
                  <div className="flex items-center space-x-3">
                    <svg className="w-5 h-5 text-light-text-tertiary dark:text-dark-text-tertiary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
